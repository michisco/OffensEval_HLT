...
from transformers import AutoModelForSequenceClassification, AutoConfig, AutoModel, AutoTokenizer, AdamW, get_linear_schedule_with_warmup
...
class BertClassifier(nn.Module):
    ...
    def __init__(self, model_name = "bert-base-uncased",
                    ...
                    dropout = 0.7):
        """
        @param    bert: an AutoModel object
...
def evaluateFinal(model, test_dataloader, device, loss_ce, label_name = ["Not Off", "Off"]):
    """After the completion of each training epoch, measure the model's performance
    on validation set. """
    # Put the model into the evaluation mode. The dropout layers are disabled during
    # the test time.
